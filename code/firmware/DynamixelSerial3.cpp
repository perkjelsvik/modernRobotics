...
{	
	...
	Time_Counter = 0;
    while((availableData() < 9) & (Time_Counter < TIME_OUT)){
		Time_Counter++;
		delayus(10);
	}
	// http://support.robotis.com/en/product/actuator/dynamixel/communication/dxl_packet.htm#Status_Packet
	/*
	Status packet: 0xFF, 0xFF, id, len, error, param1, param2...n, checksum
	In case of read present position:
	Status packet: 0xFF, 0xFF, id, len, error, posL, posH, checksum
	Length is 8:    1     2     3   4    5      6    7      8
	*/
	while (availableData() > 0){
		Incoming_Byte = readData(); // This line also removes the checksum at the end of the status response.
		if ( (Incoming_Byte == 255) & (peekData() == 255) ){
			readData();                            // Start Bytes
			readData();                            // Ax-12 ID
			readData();                            // Length
